  ...
  <div class="head fx" ref="head">
    ...
    <div v-if="isShow" class="pop" @click.stop="()=>{}">
      ...
      <div v-if="!isSuccess" class="btn fx">
        <span
          :style="{backgroundColor: inputValue.trim() ? '#2AB0EA' : '#CCCCCC'}"
          @click="useCoupons"
        >确认使用</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    toClient() {
      let id = this.getItem("userInfo").schoolUser.id;
      window.location.href = "http://192.168.3.87:8080/#/schoolDetail?id=" + id;
    }
  }
};
</script>
